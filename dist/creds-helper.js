...
export async function generateUserCredentials(username, displayName, email) {
    const config = getConfig();
    if (!config) {
        throw new Error('CredentialsHelper not configured. Call configure() before generateUserCredentials().');
    }
...
export function createCredentialsDownloadResponse(credentialsCard, username, format = 'png') {
...
export function generateCredentialsEmailHtml(credentials, includeCard = false) {
...
export function generateSecureCredentialsLink(credentialsId, expiresInMinutes = 60) {
